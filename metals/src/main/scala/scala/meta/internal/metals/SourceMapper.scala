...
final case class SourceMapper(
  ...
  def actualSourceForPc(
    ...
    def default = {
      ...
      viaBuildTargets.getOrElse((input, identity[l.Position] _, AdjustedLspData.default))
    }
    ...
    val forScripts =
      if (path.isSbt) {
        ...
        WorksheetProvider.worksheetScala3Adjustments(input, path.toURI.toASCIIString)
      } else None
